class Database {
  constructor () {
    this.worker = new Worker('sqlite.min.js', { type: 'module' })
    this.queue = []
...
async function main () {
  window.settings = { zscores: false, pvalues: false, freeze: 1, titles: {}, display: 1000 }
  const query = `-- freeze 8
-- pvalues 1
...
`
  ...
  let season = 2025
  if (url.hash) {
    season = parseInt(url.hash.slice(1), 10)
  }
  db.open(`db/summary.${season}.db`, { wasm: 'sqlite.wasm', prefetch: true })
    .then(() => {
      if (query) {
